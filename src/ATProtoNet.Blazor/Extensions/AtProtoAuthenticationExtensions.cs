    ...
    public static IEndpointRouteBuilder MapAtProtoOAuth(this IEndpointRouteBuilder endpoints)
    {
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            try
            {
                ...
                // A local return URL, or a relay URL this service has just issued: nothing else,
                // so the callback is never an open redirect.
                if (!OAuthLoginBinding.IsLocalUrl(returnUrl) && !oauthService.IsIssuedRelayUrl(returnUrl))
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            {
                if (!OAuthLoginBinding.IsLocalUrl(returnUrl))
                    returnUrl = options.DefaultReturnUrl;
                return Results.Redirect(returnUrl);
            }
