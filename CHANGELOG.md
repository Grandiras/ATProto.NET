...
- **Both `PdsService` constructors gained a trailing `IInviteCodeStore? inviteCodes = null` parameter** (Issue #36) — Source-compatible (the parameter is optional), binary-breaking: recompile against the new assembly. Behaviour also changes for anyone running with `OpenRegistration = false` — an arbitrary non-empty invite code no longer creates an account, so **issue codes through `PdsService.CreateInviteCodeAsync` (or `com.atproto.server.createInviteCode`) before closed registration will accept anyone.** When no store is supplied the service uses a private empty `InMemoryInviteCodeStore` and rejects every code, i.e. it fails closed rather than falling back to the old presence check. `OpenRegistration = true` is unaffected: invite codes are ignored, as before
...
- **`PdsSequencer.Subscribe(cursor)` and `PdsFirehoseSubscription`** (Issue #66) — Registers a firehose subscription eagerly and returns a disposable handle, instead of registering on first enumeration the way `SubscribeAsync` does. Events published between registering and the first `ReadAllAsync` read are buffered rather than missed, which is what a WebSocket handler needs: it must be subscribed before it tells the peer the stream is open. The handle also exposes the sequence state snapshotted under the same lock as the registration — `Replay`, `CurrentSeq`, `OldestAvailableSeq`, `Cursor` — so cursor validation is not racing a concurrent publish. `SubscribeAsync` is unchanged and now delegates to it
- **PDS invite codes are now a first-class, enforced feature** (Issue #36) — With `PdsOptions.OpenRegistration = false`, `PdsService.CreateAccountAsync` previously only checked that the supplied invite code was *non-empty*, so any string created an account and "invite codes are required" was not enforceable with the package alone. Codes are now stored, validated, and consumed
  - `IInviteCodeStore` — the storage seam, with `PdsInviteCode` / `PdsInviteCodeUse` models. Redemption is a three-step protocol (`TryClaimAsync` → `ConfirmClaimAsync` / `ReleaseClaimAsync`) so two concurrent sign-ups cannot double-spend the last use of a code; a database implementation should make `TryClaimAsync` a single conditional `UPDATE`. A failed sign-up (taken handle, store error) releases the reservation, leaving the code usable
  - `InMemoryInviteCodeStore` — the default implementation, registered by `AddAtProtoPds()`. Development-grade: codes are lost on restart
  - `AddAtProtoPdsInviteCodeStore<TInviteCodeStore>()` — registers a persistent invite code store, replacing the in-memory default in either call order relative to `AddAtProtoPds()`
  - `PdsService.CreateInviteCodeAsync`, `CreateInviteCodesAsync`, `GetInviteCodesAsync`, `GetAccountInviteCodesAsync`, `DisableInviteCodesAsync` — issue, list, and disable codes in-process. Generated codes follow the reference PDS's shape (`my-pds-example-com-a2b3c-d4e5f`), drawn from `RandomNumberGenerator`. `CreateInviteCodesAsync` is documented as non-atomic: arguments are validated before anything is written, but `IInviteCodeStore` has no transaction seam, so a store failure partway through a multi-account batch leaves the codes already written in place
  - Five newly mapped XRPC endpoints, all covered by `PdsEndpointNames` and therefore by `PdsEndpointOptions.Exclude`/`Only`/`Configure`: `com.atproto.server.createInviteCode`, `com.atproto.server.createInviteCodes`, `com.atproto.server.getAccountInviteCodes` (Bearer auth, returns the caller's own codes), `com.atproto.admin.getInviteCodes` (`sort=recent|usage`, `limit`, `cursor`), and `com.atproto.admin.disableInviteCodes` (by code and/or by owning account)
  - `PdsOptions.AdminPassword` / `PdsOptions.AdminUsername` (default `admin`) — the HTTP Basic credential guarding the four admin endpoints. Both halves are compared in fixed time and neither comparison is short-circuited, so response time reveals neither which half was wrong nor how much of it matched — a deployment treating a custom `AdminUsername` as a second secret is protected like the password. The endpoints **fail closed**: with no `AdminPassword` configured they reject every request with `401 AuthenticationRequired`
  - `PdsInviteCodeView` / `PdsInviteCodeUseView` — `com.atproto.server.defs#inviteCode` wire projections, plus `PdsAccountInviteCodes` for the `createInviteCodes` response
  - `docs/pds.md` gains an "Invite Codes" section covering issuing, admin auth, the claim protocol, and a SQL `TryClaimAsync` sketch
...
- **`com.atproto.sync.subscribeRepos` could miss events published right after the handshake** (Issue #66) — The endpoint accepted the WebSocket and only then subscribed to the `PdsSequencer`. Since the subscription registered lazily, on the first read of `SubscribeAsync`, a peer that acted the instant its connect returned — a relay writing to its own repo, say — could have the resulting `#commit` published into that gap, where it was in neither the replay snapshot nor the live channel. A live-only subscriber (no cursor) then waited indefinitely for a frame that was already gone. The handler now registers with the sequencer *before* accepting the handshake, so "connected" and "receiving" are the same instant. This surfaced as an intermittent 30-second timeout in `PdsFederationEndpointTests.SubscribeRepos_StreamsLiveCommitEvents` under CI load; it is reproducible on demand by delaying the handler between accept and subscribe. The cursor checks now also read the sequence state snapshotted at registration rather than the sequencer's live state, so a concurrent publish can no longer make a valid cursor look like a `FutureCursor`
...
- **Closed PDS registration was not actually closed** (Issue #36) — `PdsService.CreateAccountAsync` accepted any non-empty string as an invite code, so a PDS configured with `OpenRegistration = false` was open to anyone who sent `{"inviteCode": "x"}`. Codes are now validated against `IInviteCodeStore` and consumed atomically; see **Added** and **Breaking changes** above. Deployments that worked around this with app-side validation (terminal middleware shadowing `createAccount`) can drop it and use `PdsEndpointOptions.Exclude` if they still want their own handler
