...
- **`VideoClient.UploadVideoAsync` uploads in parts and waits for processing** — it now runs the multipart flow (`startUpload`, `uploadPart`, `finishUpload`), retries transient failures, polls the job and returns the completed `JobStatus`, whose `Blob` goes into a `VideoEmbed`; a failed job throws `VideoUploadException`. It takes a `VideoUploadOptions?` before the token. The single-request `app.bsky.video.uploadVideo` call, which returned the unprocessed job, is now `UploadVideoInOneRequestAsync`. Migration: use the returned `JobStatus` directly and drop the polling loop, or call `UploadVideoInOneRequestAsync` to keep the old behaviour (#127)
- **`PostView` writes its `$type`** — `PostView` and `ThreadNode` now derive from the new `PostEntry` union base (see *Added*), so a serialized `PostView` leads with `"$type":"app.bsky.feed.defs#postView"`, as `GeneratorView` and the other union members already do. Migration: none for reading; recompile, and expect the `$type` in JSON you write from a `PostView` (#127)
...
- **Verified record reads (`com.atproto.sync.getRecord`)** — `SyncClient.GetRecordAsync` downloads a record's proof CAR, and `GetVerifiedRecordAsync`, or `RecordProof.Verify` over a CAR you already hold, checks it against the repository's signed commit: block CIDs, the commit's DID and signature, and the tree path to the key. The result is a `VerifiedRecord` (the record, its CID, the commit and revision) or proof that the record does not exist; anything else throws the new `RepoVerificationException`. It is tested against proofs produced by the reference implementation (#129)
- **`SyncClient.GetBlocksAsync` and `RepoClient.ImportRepoAsync`** — `com.atproto.sync.getBlocks` fetches repository blocks by CID, and `com.atproto.repo.importRepo` uploads a repository CAR into the signed-in account, the step of an account migration that moves its records (#129)
- **`AdminClient.SearchAccountsAsync` / `EnumerateSearchAccountsAsync` and `UpdateAccountSigningKeyAsync`** — `com.atproto.admin.searchAccounts` and `updateAccountSigningKey`. `PdsAdminClient` gains `SearchAccountsAsync` / `EnumerateSearchAccountsAsync` for the PDSs that serve it, such as Tranquil; the reference PDS implements neither method (#129)
- **`AtProtoClient.Temp` (`com.atproto.temp.*`)** — `CheckHandleAvailabilityAsync` (with suggestions when the handle is taken), `CheckSignupQueueAsync`, `DereferenceScopeAsync` (expands a `ref:` OAuth scope reference), `RequestPhoneVerificationAsync` and `RevokeAccountCredentialsAsync` (#129)
- **`site.standard.graph.recommend`** — `RecommendRecord`, and `StandardSiteClient.CreateRecommendationAsync` / `GetRecommendationAsync` / `DeleteRecommendationAsync` / `ListRecommendationsAsync` / `EnumerateRecommendationsAsync` (#129)
- **Bookmarks** — `client.Bsky.Bookmark` creates, deletes and lists the account's private bookmarks (`app.bsky.bookmark.*`), with `EnumerateBookmarksAsync`. Each `BookmarkView.Item` is a `PostEntry`: the new open union of `PostView`, `NotFoundPost` and `BlockedPost` (#127)
- **Multipart video upload** — `VideoClient.StartUploadAsync`, `UploadPartAsync`, `FinishUploadAsync`, `GetUploadStatusAsync` and `AbortUploadAsync` (`app.bsky.video.*`, for videos up to 300 MB), with `UploadState` and the upload error names in `VideoErrors`. `UploadVideoAsync` drives them, aborting the session when it fails or is cancelled (#127)
- **`FeedClient.SearchPostsV2Async`** — `app.bsky.feed.searchPostsV2` with every filter in a `PostSearchFilters` (authors, mentions, domains, URLs, embedded records, hashtags and languages to include or exclude, media, replies, thread, `Following`, `QueryLanguage`, …), `HitsTotal` and `DetectedQueryLanguages`, plus `EnumerateSearchPostsV2Async` (#127)
- **Notification preferences, activity subscriptions and `unregisterPush`** — `NotificationClient.GetPreferencesAsync` / `PutPreferencesV2Async` (`NotificationPreferences`), `ListActivitySubscriptionsAsync` / `EnumerateActivitySubscriptionsAsync` / `PutActivitySubscriptionAsync`, `UnregisterPushAsync` (with `PushPlatform`), and the `NotificationDeclarationRecord` that says who may subscribe (#127)
- **Actor and graph records** — `StatusRecord` (`app.bsky.actor.status`, with `ActorStatus.Live`), `ContentVisibilityDeclarationRecord`, `VerificationRecord` and `ReferenceListOptOutRecord` (#127)
- **Graph membership and starter pack search** — `GraphClient.GetListsWithMembershipAsync` and `GetStarterPacksWithMembershipAsync` list the viewer's lists and starter packs with an actor's membership, and `SearchStarterPacksV2Async` returns full starter pack views; each has an `Enumerate*` method (#127)
- **Drafts** — `client.Bsky.Draft` stores, updates, deletes and lists the account's private post drafts (`app.bsky.draft.*`) (#127)
- **Post threads as the Bluesky app loads them** — `client.Bsky.Unspecced.GetPostThreadV2Async` returns a flat, depth-annotated thread (`ThreadItem`, with `ThreadItemPost` and its placeholders), and `GetPostThreadOtherV2Async` the replies it leaves out (#127)
- **Enhanced link cards** — `client.Bsky.Embed.GetEmbedExternalViewAsync` resolves the records behind a web page into an external embed view and the references to attach to the post (#127)
- **Feed feedback** — `FeedClient.SendInteractionsAsync` sends `Interaction`s (`InteractionEvent`) to a feed generator, proxied to it through the new `ServiceProxy.BskyFeedGenerator` when its DID is given (#127)
- **Age assurance** — `client.Bsky.AgeAssurance` reads the regional configuration (`AgeAssuranceConfig`, with its rules as an open union) and the account's state, and begins the process (`app.bsky.ageassurance.*`) (#127)
