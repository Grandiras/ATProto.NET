...
- **`Session.Did` and `Session.Handle` are typed and `required`** — a session applied from an OAuth login whose handle did not verify carries `handle.invalid` rather than the DID. Migration: `new Session { Did = Did.Parse(…), Handle = Handle.Parse(…), … }`; read `OAuthSessionResult.Handle` / `IsHandleVerified` for the raw value (#119)
